...
/*Este codigo funciona apenas para o sensor DHT11 com o sensor ligado na porta analogica A1 e esta configurado para transmitir suas leituras , para  utilizar com outros sensores será necessario alterar o codigo 
 * e as funcionalidades do mesmo. 
 * Codigo desenvolvido por Ivan Obeidi para o trabalho de conclusão de curso. 
 */
#include <RF24Network.h> 
#include <RF24.h> 
#include <SPI.h>
#include <EEPROM.h>
#include <DHT.h> // inclui a biblioteca para trabalhar com sensores dht
#define DHTPIN A1 //<--- define o pino que esta  ligado o sensor
#define DHTTYPE DHT11 //<--- define qual DHT está ligado


DHT dht(DHTPIN, DHTTYPE); //<-- inicializa o objeto dht para comunicar com o sensor
...
void loop() {
  ...
  if(timer)
  {           
    payload_t payload ={endNo,sIntervalo,Modo,tipo,LerTemp(),LerUm()}; //faz o pacote a ser transmitido
...
float LerTemp()//faz leitura da temperatura
{
  TempT = dht.readTemperature();
  return TempT ;   
}
float LerUm() //<--- Faz leitura da umidade
{
  HumT =  dht.readHumidity(); 
  return HumT ; 
 }
