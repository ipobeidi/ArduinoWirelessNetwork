/*Este codigo funciona apenas para o Sensor Ultrasonico com o sensor ligado nas portas 5 (trigger) e 4 (echo)  e esta configurado para transmitir suas leituras , para  utilizar com outros sensores será necessario alterar o codigo 
...
#include <SPI.h>
#include <EEPROM.h>
#include <Ultrasonic.h> // inclui a biblioteca para trabalhar com sensores ultrasonicos
#define TRIGGER_PIN 5 //<--- define o pino triger do  sensor
#define ECHO_PIN 4 //<--- define o pino eco

Ultrasonic ultrasonic(TRIGGER_PIN, ECHO_PIN); //<-- inicializa o objeto ultrasonic para comunicar com o sensor
...
void loop() {
  ...
  if(timer)
  {           
    payload_t payload ={endNo,sIntervalo,Modo,tipo,LerDistancia(),0}; //faz o pacote a ser transmitido
...
float LerDistancia()//faz leitura da temperatura
{
